...
const taskElemenets = document.querySelector('.board__tasks');
const NEW_NUMBER = 4;
...
// Функция отрисовки фильтров
    const renderFilterElements = () => {
      ...
      filterElements.insertAdjacentHTML(`beforeend`, getFilterElement(nameFilter[0], getRandomInRange(1, 100), true));

      for (let i = 1; i < nameFilter.length; i++) {
        filterElements.insertAdjacentHTML(`beforeend`, getFilterElement(nameFilter[i], getRandomInRange(1, 100)));
      }
    };
...
//Функция отрисовки карточек товара
    const renderTasksElement = (number) => {
      for (let i = 0; i < number; i++) {
        taskElemenets.insertAdjacentHTML(`beforeend`, getTaskElment());
      }
    };
...
//Переключение фильтров
    const filterClickHandler = function (event) {
      taskElemenets.innerHTML = "";
